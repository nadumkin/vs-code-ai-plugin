...
class ToolExecutor {
  constructor(outputChannel, memoryManager = null, logger = null) {
  ...
  async executeToolCall(toolCall, hooks = {}) {
    ...
    this.logger.append("tool_call", { id, name, args });
    ...
    switch (name) {
      ...
      this.logger.append("tool_result", {
        id,
        name,
        ms: Date.now() - startedAt,
        ok: !logError && Boolean(result?.ok),
        summary: result?.summary || (logError ? logError.message : null),
        requiresApproval: Boolean(result?.requiresApproval),
        error: logError ? String(logError.message || logError) : null,
      });
    }
