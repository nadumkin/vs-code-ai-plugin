...
const { sanitizeAssistantText, isLikelyInternalSystemMessage } = require("../util/DisplaySanitizer");
...
class AssistantViewProvider {
  ...
  constructor(context, runtime, openRouterClient, logger = null) {
  ...
  async handleLoadModels() {
    ...
    this.busy = true;
    this.status = "Загружаю список моделей...";
    this.postState();

    try {
      const models = await this.openRouterClient.listModels();
      this.availableModels = Array.isArray(models) ? models : [];
      this.postInfo(`Доступно моделей: ${this.availableModels.length}.`);
      return true;
    } catch (error) {
      this.messages.push(
        this.createMessage(
  ...
  async handleSubmitPrompt(rawPrompt) {
    ...
    this.logger.append("user_prompt", { prompt, fromDefault: !String(rawPrompt || "").trim() });
  ...
  async runAgentTurn({ prompt = "", history = [], iterationLimit, continuationState }) {
    ...
    if (result.status === "completed") {
      ...
      this.pendingApproval = undefined;
      this.approvalMessage = "";
      const sanitized = sanitizeAssistantText(result.assistantMessage);
      this.logger.append("agent_final_message", {
        rawLength: String(result.assistantMessage || "").length,
        displayLength: sanitized.length,
      });
      this.messages.push(this.createMessage("assistant", sanitized));
      return;
    }
  ...
  handleToolEvent(event) {
    if (event.displayMessage) {
      // rich visual cards (commands, file changes, memory predictions) always show
      this.upsertMessage(this.createMessage(event.displayMessage.kind, "", event.displayMessage));
    } else if (event.summary) {
      const text = String(event.summary || "");
      // operational tool summaries ("Прочитан файл…", "Поиск…", "Memory: …",
      // "Промежуточный ответ агента…") are log-only by default — they are already
      // captured in the JSONL log via tool_result/assistant_message events.
      const showActivity = this.getShowToolActivity();
      if (showActivity && !isLikelyInternalSystemMessage(text)) {
        this.messages.push(this.createMessage("system", text));
      }
    }
    this.postState();
  ...
  getShowToolActivity() {
    return Boolean(
      vscode.workspace
        .getConfiguration("aiAgentAssistant")
        .get("display.showToolActivity", false)
    );
  }
        ...
        <div class="settings-grid">
          ...
          <label class="settings-field">
            ...
            <div class="settings-model-row">
              <select id="settingsModel" class="settings-input"></select>
              <button id="loadModels" type="button" class="ghost">Загрузить модели</button>
            </div>
            <span class="settings-note">Список приходит с сервера (Proxy /v1/models). Кастомную модель можно задать в settings.json.</span>
          </label>
