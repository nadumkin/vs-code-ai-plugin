(function () {
  ...
  function populateModelSelect() {
    const select = elements.settingsModel;
    if (!select) {
      return;
    }
    ...
    // Preserve the current selection and the configured model so nothing is lost.
    const desired = select.value || state.modelName || "";
    add(desired);
    add(state.modelName);
    ...
    if (ids.length === 0) {
      ...
      opt.value = "";
      opt.textContent = "— нажмите «Загрузить модели»";
    ...
    ids.forEach((id) => {
      const opt = document.createElement("option");
      opt.value = id;
      opt.textContent = adapters[id] ? `${id} (${adapters[id]})` : id;
      select.appendChild(opt);
    });
    select.value = desired || ids[0];
  }
  ...
  function render() {
    ...
    elements.loadModels.disabled = state.busy;
  ...
  function renderMessage(message) {
    ...
    body.textContent = sanitizeForDisplay(message.content, message.role);
  ...
  // Defensive: strip <think>/<thinking>/<reflection>/<scratchpad> and internal
  // markers in case anything slipped through the host-side sanitizer.
  const THINK_BLOCK_RE = /<(think|thinking|reflection|scratchpad|internal|scratch)\b[^>]*>[\s\S]*?<\/\1>/gi;
  const INTERNAL_LINE_RE = /^\s*\[internal\][^\n]*\n?/gm;
  const MEMORY_ECHO_RE = /^\s*\[Memory of past similar changes\][\s\S]*?(?:\n\s*\n|$)/i;
  const REQUEST_TYPE_RE = /^\s*\[request_type=[^\]]*\]\s*\n?/i;

  function sanitizeForDisplay(text, role) {
    let s = String(text == null ? "" : text);
    if (role === "assistant" || role === "system") {
      s = s.replace(THINK_BLOCK_RE, "");
      s = s.replace(MEMORY_ECHO_RE, "");
      s = s.replace(REQUEST_TYPE_RE, "");
      s = s.replace(INTERNAL_LINE_RE, "");
      s = s.replace(/\n{3,}/g, "\n\n").trim();
    }
    return s;
  }
