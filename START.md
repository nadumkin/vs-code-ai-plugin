...
3. В этом окне открыть панель агента (значок **Agent** на activity bar) → **AI Agent: Open Settings** (значок «Настройки»):
   ...
   - выбрать модель: `mock/echo` (мгновенно, офлайн) или реальную локальную `qwen/qwen2.5-coder-0.5b`
     (первый запрос скачивает ~1 ГБ и медленнее на CPU, дальше — из кэша);
   - в поле **«Ключ доступа (Proxy Service)»** вписать токен — по умолчанию `dev-secret-token-123` (из `server/tokens.json`). Ключ OpenRouter здесь **не нужен** — он задаётся на сервере (`OPENROUTER_KEY`);
   - **Сохранить**.
4. Ввести запрос и отправить — ответ придёт через бэкенд. `mock/echo` вернёт текст-эхо; `qwen/qwen2.5-coder-0.5b` — ответ реальной модели.
...
| Локальная модель «думает» долго / таймаут при ответе | 0.5B на CPU медленная, особенно с большим контекстом (промпт >120 с раньше обрывался). Таймаут ответа поднят до 10 мин (`aiAgentAssistant.backend.responseTimeoutMs` в плагине, `ws_response_timeout` на сервере). Для скорости уменьшите контекст: `aiAgentAssistant.context.maxFileChars` / `maxImportedFiles` / `maxTests`, либо используйте `openrouter`/GPU. После правок кода плагина — **Cmd+R** в окне Extension Development Host. |
| Локальная модель не вставляет код / в чат течёт JSON вызова инструмента | 0.5B нестабильно ведёт tool-loop (зацикливается, выдумывает инструменты, шлёт «сырой» JSON). Поэтому для встроенной `qwen/qwen2.5-coder-0.5b` инструменты **отключены на сервере** (`supports_tools: false` в `models.json`): модель возвращает цельный файл кодом, и плагин применяет его как замену **активного файла** через **diff-подтверждение** (для всех запросов, кроме «объясни/вопрос»). Нужен открытый активный файл; после обновления кода плагина — **Cmd+R**. Глобально клиентом инструменты выключаются через `aiAgentAssistant.agent.useTools: false`. ⚠️ Качество правок ограничено размером модели (0.5B часто не выполняет инструкцию точно) — для надёжного результата используйте `openrouter` или модель крупнее. |
